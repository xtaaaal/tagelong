// CSV Importer Bootstrap Script
// Run this with: npm run strapi import:csv path/to/csv/file.csv
...
class ItineraryCSVImporter {
  ...
  async importItinerary(row) {
    ...
    const existingItinerary = await this.strapi.documents('api::itinerary.itinerary').findFirst({
      filters: {
    ...
    const createdItinerary = await this.strapi.documents('api::itinerary.itinerary').create({
      data: itineraryData,
      status: 'draft'
    });
...
module.exports = {
  /**
   * CSV Import function that can be called from Strapi bootstrap or manually
   */
  async importCSV(strapi, csvFilePath) {
    console.log('🚀 Starting CSV Import...');
    
    const importer = new ItineraryCSVImporter(strapi);
    await importer.importFromCSV(csvFilePath);
    
    console.log('🎉 CSV Import completed!');
  },

  ItineraryCSVImporter
};
